...
namespace UnityStandardAssets.Characters.FirstPerson
{
	...
	public class FirstPersonController_Egen : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if (!playerGetCaught)
			{
				if (!menuEnabled && Input.GetKeyDown(KeyCode.JoystickButton10) && !playerInhole)
				{
					((playerCrawl)crouchHolder.GetComponent(typeof(playerCrawl))).crouching();
				}
				if (Input.GetKeyDown(KeyCode.JoystickButton7) && !menuEnabled)
				{
