...
public class openCloseVitrindoorL : MonoBehaviour
{
	...
	{
		if (!DoorOpen && GetComponent<Animation>().IsPlaying("VitrindoorHallLOpen"))
		{
		...
		if (DoorOpen && GetComponent<Animation>().IsPlaying("VitrindoorHallLClose"))
		{
			GetComponent<AudioSource>().PlayOneShot(doorLjud);
			GetComponent<Collider>().enabled = false;
			DoorOpen = false;
	...
	public virtual IEnumerator timerDooropen()
	{
		...
		base.gameObject.tag = "vitrindoorLopen";
	...
	{
		yield return new WaitForSeconds(1f);
		base.gameObject.tag = "vitrindoorLclosed";
		Innerdoor.carving = !base.enabled;
		GetComponent<Collider>().enabled = true;
	}
}
