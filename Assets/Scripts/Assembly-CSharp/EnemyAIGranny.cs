					...
					if (!hitByArrow && !hitByGun && !bastuKilled && !hitByCar)
					{
						...
						else if (hitInfo.collider.gameObject.tag == "smalldoorClosed")
						{
							...
							if (seeClosedDoorTimer >= 2f)
							{
								...
								hitInfo.collider.gameObject.GetComponent<Animation>().Play("SmallDoorOpen_0");
								hitInfo.collider.gameObject.GetComponent<Animation>().Play("SmallDoorOpen_1");
								hitInfo.collider.gameObject.GetComponent<Animation>().Play("SmallDoorOpen_2");
								hitInfo.collider.gameObject.GetComponent<Animation>().Play("SmallDoorOpen_3");
								if (navComponent.velocity != Vector3.zero)
								{
									animationHolder.GetComponent<Animation>().CrossFade("Walk");
								}
								...
								if (!seePlayer)
								{
									if (playerHiding)
									{
										...
										if (seeClosedDoorTimer >= 2f)
										{
											...
											hitInfo.collider.gameObject.GetComponent<Animation>().Play("Garde1Open_2");
											if (navComponent.velocity != Vector3.zero)
											{
												animationHolder.GetComponent<Animation>().CrossFade("Walk");
											}
