...
public class PickUp : MonoBehaviour
{
	...
	{
		//layerMask = 10;
		Debug.Log("hmmm wonder why this ran"); 
	}

	public virtual void Start()
	{
		//layerMask = ~layerMask;
		Debug.Log("no idea what this does anyways");
	}
	...
	public virtual void Update()
	{
		...
		else if ((Input.GetKeyDown(KeyCode.JoystickButton1) || Input.GetKeyDown(KeyCode.Space)) && !((FirstPersonController_Egen)player.GetComponent(typeof(FirstPersonController_Egen))).menuEnabled && dropObjectButton.activeSelf)
		{
			dropObject = true;
		}
		...
		if (!playerTaken)
		{
			if (Physics.Raycast(SeeRay.transform.position, direction, out hitInfo, 5f/*, layerMask*/))
			{
				if (hitInfo.collider.gameObject.tag == "avbitare")
				{
					...
					{
						pickUp = false;
						UnityEngine.Object.Destroy(hitInfo.collider.transform.parent.gameObject);
