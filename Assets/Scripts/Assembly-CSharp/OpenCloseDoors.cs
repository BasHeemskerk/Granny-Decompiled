using System;
using System.Collections;
using UnityEngine;
using UnityEngine.AI;

[Serializable]
public class OpenCloseDoors : MonoBehaviour
{
	public bool DoorOpen;

	public bool DoorMoving;

	public NavMeshObstacle Innerdoor;

	public AudioClip doorOpen;

	public AudioClip doorClose;

	public AudioClip doorLockedLjud;

	public AudioClip doorUnLockedLjud;

	public bool doorLocked;

	public NavMeshObstacle obs;

	public virtual void Start()
	{
		if (PlayerPrefs.GetInt("DiffData") == 4)
		{
			obs = GetComponent<NavMeshObstacle>();
			obs.enabled = false;
		}
	}

	public virtual void Update()
	{
		if (!DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorOpen") || !DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorOpen_0") || !DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorOpen_1") || !DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorOpen_2") || !DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorOpen_3"))
		{
			if (base.gameObject.tag == "innerdoorLocked")
			{
				GetComponent<AudioSource>().PlayOneShot(doorUnLockedLjud);
			}
			else
			{
				GetComponent<AudioSource>().PlayOneShot(doorOpen);
			}
			DoorOpen = true;
			DoorMoving = true;
			base.gameObject.tag = "Untagged";
			Innerdoor.carving = base.enabled;
			StartCoroutine(timerDooropen());
		}
		if (DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorClose") || DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorClose_0") || DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorClose_1") || DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorClose_2") || DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorClose_3"))
		{
			GetComponent<AudioSource>().PlayOneShot(doorClose);
			DoorOpen = false;
			DoorMoving = true;
			base.gameObject.tag = "Untagged";
			StartCoroutine(timerDoorclosed());
		}
		if ((!DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorLocked") && !doorLocked) || !DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorLocked_0") || (!DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorLocked_1") && !doorLocked) || (!DoorOpen && GetComponent<Animation>().IsPlaying("InnerdoorLocked_2") && !doorLocked))
		{
			doorLocked = true;
			GetComponent<AudioSource>().PlayOneShot(doorLockedLjud);
			StartCoroutine(timerDoorlocked());
		}
	}

	public virtual IEnumerator timerDooropen()
	{
		yield return new WaitForSeconds(1f);
		base.gameObject.tag = "innerdoorOpen";
		DoorMoving = false;
	}

	public virtual IEnumerator timerDoorclosed()
	{
		yield return new WaitForSeconds(1f);
		base.gameObject.tag = "innerdoorClosed";
		Innerdoor.carving = !base.enabled;
		doorLocked = false;
		DoorMoving = false;
	}

	public virtual IEnumerator timerDoorlocked()
	{
		yield return new WaitForSeconds(1f);
		doorLocked = false;
	}

	public virtual void OnTriggerEnter(Collider other)
	{
		if (DoorMoving)
		{
			if (other.gameObject.tag == "Player")
			{
				Physics.IgnoreCollision(GetComponent<Collider>(), other.GetComponent<Collider>(), true);
			}
		}
		else if (!DoorMoving)
		{
			Physics.IgnoreCollision(GetComponent<Collider>(), other.GetComponent<Collider>(), false);
		}
	}
}
